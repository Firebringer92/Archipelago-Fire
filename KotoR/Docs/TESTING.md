...
**Use `!`, not `/`, for the commands above** — `!` is this client's own
custom-command prefix; `/` is reserved for `CommonClient`'s built-ins
(`/connect`, etc.) and won't recognize them. If you're running the client
from a bash-family shell, be aware `!` also triggers bash's own history
expansion (`event not found` errors) — either run from `cmd.exe`/
PowerShell instead, or run `set +H` in bash first to disable it.
