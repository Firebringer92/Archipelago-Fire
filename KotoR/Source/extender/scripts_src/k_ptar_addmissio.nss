// Suppression wrapper for mission's real story recruitment script
// (originally tar_m04aa's k_ptar_addmissio, preserved as apo_mission_orig).
//
...
//
// GUARD (added 2026-08-31): if NPC_MISSION is ALREADY available when this
// fires, our own companion_mission AP arm already ran (early item, or an
// admin re-grant) and mission is already an active party member. Confirmed
// live: running apo_mission_orig + RemoveAvailableNPC on an
...
void main()
{
    ...
    {
        ExecuteScript("apo_mission_orig", OBJECT_SELF);
        RemoveAvailableNPC(NPC_MISSION);
        KSE_Diag(32, "AP|SUPPRESSED|companion_mission");
    }
    else
    {
        KSE_Diag(32, "AP|SUPPRESSED|companion_mission|skipped_already_recruited");
    }
    ...
    KSE_Diag(22, "AP|CHECK|COMPANION|6");
}
