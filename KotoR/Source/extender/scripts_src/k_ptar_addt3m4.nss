// Suppression wrapper for t3m4's real story recruitment script
// (originally tar_m02ab's k_ptar_addt3m4, preserved as apo_t3m4_orig).
//
...
//
// GUARD (added 2026-08-31): if NPC_T3_M4 is ALREADY available when this
// fires, our own companion_t3m4 AP arm already ran (early item, or an
// admin re-grant) and t3m4 is already an active party member. Confirmed
// live: running apo_t3m4_orig + RemoveAvailableNPC on an
...
void main()
{
    ...
    {
        ExecuteScript("apo_t3m4_orig", OBJECT_SELF);
        RemoveAvailableNPC(NPC_T3_M4);
        KSE_Diag(33, "AP|SUPPRESSED|companion_t3m4");
    }
    else
    {
        KSE_Diag(33, "AP|SUPPRESSED|companion_t3m4|skipped_already_recruited");
    }
    ...
    KSE_Diag(22, "AP|CHECK|COMPANION|7");
}
